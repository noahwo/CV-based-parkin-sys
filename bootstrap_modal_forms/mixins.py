...
class CreateUpdateAjaxMixin(object):
    ...
    def save(self, commit=True):
        # if not self.request.headers.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest' or self.request.POST.get('closeOnSubmit') == 'False':
        if not self.request.headers.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest' or self.request.POST.get('asyncUpdate') == 'True':
